...
def main():
    ...
    model_resume_path = './record/spatial/resnet_ucf101_org/model_best.pth.tar'
    ...
    annot_img_index_list, entire_img_index_list, all_frames, all_frame_names, all_labels = load_frames(new_img_list = "new_file_with_start_end_frame_valid.txt",
                                            img_list = "./thumos14_list/final_thumos_14_20_one_label_temporal_valid_with_img_num.txt",
                                            video_root_path = "/media/dataDisk/THUMOS14/THUMOS14_video/thumos14_preprocess/valid/frames_10fps",
                                            num_frames=30)

    feature_dir = "./saved_features/valid"
    ...
    for i in range(toal_num_video):
        ...
        #save_num =i+7571
        save_num = i
        np.save(os.path.join(feature_dir, 'features_{}.npy'.format('%05d'%save_num)), features_np)
        np.save(os.path.join(feature_dir, 'name_{}.npy'.format('%05d'%save_num)), all_frame_names[i])
        np.save(os.path.join(feature_dir, 'label_{}.npy'.format('%05d'%save_num)), per_video_label)
