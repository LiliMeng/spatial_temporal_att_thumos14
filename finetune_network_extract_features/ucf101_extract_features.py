    ...
    for i in range(0, num_frames):
        ...
        if total_num_imgs_per_video < num_frames:
            if i >= len(img_index_list):
                img_name = os.path.join(video_path, 'frame' + str('%06d'%(img_index_list[-1])) + '.jpg')
        	else:
...
def main():
 ...
 #                                                num_frames=50)
	...
	feature_dir = "./saved_features/ucf101_train"
	...
	img_list = "./ucf101_list/new_ucf101_train_list.txt"
	...
	for line in lines:
		...
		imgs_per_video_list, label, imgs_names_per_video_list = load_frame_per_video(line, "/media/dataDisk/THUMOS14/UCF101_jpegs_256/", 50)
